// renderer 图标单一真相（CP4）：全仓内联 SVG 收敛于此，新增齿轮/⋯。
// 不图标化纪律（ui-scan）：连接状态文字、分区标题、危险操作、保存/取消保留文字。
// 图标走 currentColor，随 token 语义类染色。
...
export function IconStarOutline({ className = "h-4 w-4" }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
      <path
        strokeLinecap="round"
...
export function IconStarFilled({ className = "h-4 w-4" }: { className?: string }) {
  return (
...
export function IconClearText({ className = "h-4 w-4" }: { className?: string }) {
  return (
